...
class TensorFile {
   public:
    TensorFile(std::string filename) {
        this->file_ = fopen(filename.c_str(), "r");
    }
    ~TensorFile() { fclose(this->file_); }
    ...
    void load_tensor_dense(std::string tensor_name, char *tensor,
    ...
    void load_tensor_sparse(std::string tensor_name, char *values,
...
template <
    dalotia_WeightFormat weight_format, typename value_type = char,
...
load_tensor_dense(std::string filename, std::string tensor_name,
    ...
    std::pmr::vector<int> true_extents(
        long_extents.begin(), long_extents.begin() + num_nonzero, alloc);
    auto total_size = std::accumulate(true_extents.begin(), true_extents.end(),
                                      1, std::multiplies<size_t>());
    std::pmr::vector<char> tensor(alloc);
...
extern "C" int load_tensor_dense(DalotiaTensorFile *file,
    ...
    if (format == dalotia_float_32 && ordering == dalotia_C_ordering) {
        reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_dense<dalotia_float_32, dalotia_C_ordering>(
                tensor_name, tensor);
    } else if (format == dalotia_float_16 && ordering == dalotia_C_ordering) {
        reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_dense<dalotia_float_16, dalotia_C_ordering>(
                tensor_name, tensor);
    } else if (format == dalotia_float_8 && ordering == dalotia_C_ordering) {
        reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_dense<dalotia_float_8, dalotia_C_ordering>(
                tensor_name, tensor);
    } else if (format == dalotia_bfloat_16 && ordering == dalotia_C_ordering) {
        reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_dense<dalotia_bfloat_16, dalotia_C_ordering>(
                tensor_name, tensor);
    } else if (format == dalotia_int_2 && ordering == dalotia_C_ordering) {
        reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_dense<dalotia_int_2, dalotia_C_ordering>(tensor_name,
                                                                   tensor);
    } else {
        return 1;
    }
    ...
    if (format == dalotia_SparseFormat::dalotia_CSR &&
        ...
        return reinterpret_cast<dalotia::TensorFile *>(file)
            ->load_tensor_sparse<dalotia_SparseFormat::dalotia_CSR,
                ...
                tensor_name, values, first_indices, second_indices);
    }
}
